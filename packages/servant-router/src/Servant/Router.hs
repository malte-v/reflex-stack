...
instance (HasRouter x, HasRouter y) => HasRouter (x :<|> y) where
  ...
  constHandler _ m a =
    constHandler (Proxy :: Proxy x) m a
...
instance
  (HasRouter sublayout, FromHttpApiData x) =>
  HasRouter (Capture sym x :> sublayout)
  where
...
instance
  (HasRouter sublayout, FromHttpApiData x, KnownSymbol sym) =>
  HasRouter (QueryParam sym x :> sublayout)
  where
  type
    RouteT (QueryParam sym x :> sublayout) m a =
...
instance
  (HasRouter sublayout, FromHttpApiData x, KnownSymbol sym) =>
  HasRouter (QueryParams sym x :> sublayout)
  where
...
instance
  (HasRouter sublayout, KnownSymbol sym) =>
  HasRouter (QueryFlag sym :> sublayout)
  where
...
instance
  (HasRouter sublayout, KnownSymbol path) =>
  HasRouter (path :> sublayout)
  where
